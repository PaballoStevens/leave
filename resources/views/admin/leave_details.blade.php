                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <div class="modal-footer justify-content-center">
                                <button class="btn btn-primary" id="action_take" >Take&nbsp;Action</button>
                            </div>
                        </div>
                    </div>
...
<script>
    const action_take = document.querySelector('#action_take');
    const form = document.querySelector('#form');

    action_take.addEventListener('click' , function () {
        form.innerHTML = '<div class="modal-dialog modal-dialog-centered" role="document"><div class="modal-content"><div class="modal-body text-center font-18"><h4 class="mb-20">Leave take action</h4><input type="hidden" name="id" value="{{$leave->id}}"/><input type="hidden" name="empid" value="{{$leave->empid}}"/><input type="hidden" name="email" value="{{$leave->email}}"/><input type="hidden" name="name" value="{{$leave->FirstName}} {{$leave->LastName}}"/><input type="hidden" name="Av_leave" value="{{$leave->Av_leave}}"><input type="hidden" name="num_days" value="{{$leave->num_days}}"><select name="Status" required class="custom-select form-control"><option value="">Choose your option</option><option value="1">Approved</option><option value="2">Rejected</option></select><div class="form-group"><label></label><textarea id="textarea1" name="AdminRemark" class="form-control" required placeholder="Description" length="300" maxlength="300"></textarea></div></div><div class="modal-footer justify-content-center"><input type="submit" class="btn btn-primary" name="update" value="Submit"></div></div></div>';
    })
</script>
